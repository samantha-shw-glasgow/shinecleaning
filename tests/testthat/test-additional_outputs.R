...
test_that("Excels return values matching report - secondary", {
  ...
  expected <- tibble::tibble(
    ...
    `Choice` = summary_mean_multiple_vars(sec_test_a, list(lifesat4 = "Choice"))[[1]]$mean,
    `Friends` = summary_mean_multiple_vars(sec_test_a, list(lifesat5 = "Friends"))[[1]]$mean,
    `Things you have` = summary_mean_multiple_vars(sec_test_a, list(lifesat6 = "Things you have"))[[1]]$mean,
    `Health` = summary_mean_multiple_vars(sec_test_a, list(lifesat7 = "Health"))[[1]]$mean,
    `Appearance` = summary_mean_multiple_vars(sec_test_a, list(lifesat8 = "Appearance"))[[1]]$mean,
    `Future` = summary_mean_multiple_vars(sec_test_a, list(lifesat9 = "Future"))[[1]]$mean,
    `School` = summary_mean_multiple_vars(sec_test_a, list(lifesat10 = "School"))[[1]]$mean,
    `Time use` = summary_mean_multiple_vars(sec_test_a, list(lifesat11 = "Time use"))[[1]]$mean,
    `% low: Overall` = summary_proportions_multiple(sec_test_a, list(lifesat1 = "Overall"), ~ valid_numbers(.x) < 5)[[1]]$prop * 100,
    `% low: Family` = summary_proportions_multiple(sec_test_a, list(lifesat2 = "Family"), ~ valid_numbers(.x) < 5)[[1]]$prop * 100,
    `% low: Home` = summary_proportions_multiple(sec_test_a, list(lifesat3 = "Home"), ~ valid_numbers(.x) < 5)[[1]]$prop * 100,
    `% low: Choice` = summary_proportions_multiple(sec_test_a, list(lifesat4 = "Choice"), ~ valid_numbers(.x) < 5)[[1]]$prop * 100,
    `% low: Friends` = summary_proportions_multiple(sec_test_a, list(lifesat5 = "Friends"), ~ valid_numbers(.x) < 5)[[1]]$prop * 100,
    `% low: Things you have` = summary_proportions_multiple(sec_test_a, list(lifesat6 = "Things you have"), ~ valid_numbers(.x) < 5)[[1]]$prop * 100,
    `% low: Health` = summary_proportions_multiple(sec_test_a, list(lifesat7 = "Health"), ~ valid_numbers(.x) < 5)[[1]]$prop * 100,
    `% low: Appearance` = summary_proportions_multiple(sec_test_a, list(lifesat8 = "Appearance"), ~ valid_numbers(.x) < 5)[[1]]$prop * 100,
    `% low: Future` = summary_proportions_multiple(sec_test_a, list(lifesat9 = "Future"), ~ valid_numbers(.x) < 5)[[1]]$prop * 100,
    `% low: School` = summary_proportions_multiple(sec_test_a, list(lifesat10 = "School"), ~ valid_numbers(.x) < 5)[[1]]$prop * 100,
    `% low: Time use` = summary_proportions_multiple(sec_test_a, list(lifesat11 = "Time use"), ~ valid_numbers(.x) < 5)[[1]]$prop * 100,
    `% reporting low mood` = extract_perc_vals(sec_test_a, who_cat, create_collapsed_summary, "low"),
    `% reporting good mood` = extract_perc_vals(sec_test_a, who_cat, create_collapsed_summary, "good"),
