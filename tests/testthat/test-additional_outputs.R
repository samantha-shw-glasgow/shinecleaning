...
test_that("Excels return values matching report - primary", {
  ...
  expected <- tibble::tibble(
    ...
    `Time use` =
     ...
     `% low: Overall` =
      summary_proportions_multiple(pri_test_a, list(lifesat1 = "Overall"), ~ valid_numbers(.x) < 5)[[1]]$prop * 100,
     `% low: Family` =
      summary_proportions_multiple(pri_test_a, list(lifesat2 = "Family"), ~ valid_numbers(.x) < 5)[[1]]$prop * 100,
     `% low: Home` =
      summary_proportions_multiple(pri_test_a, list(lifesat3 = "Home"), ~ valid_numbers(.x) < 5)[[1]]$prop * 100,
     `% low: Choice` =
      summary_proportions_multiple(pri_test_a, list(lifesat4 = "Choice"), ~ valid_numbers(.x) < 5)[[1]]$prop * 100,
     `% low: Friends` =
      summary_proportions_multiple(pri_test_a, list(lifesat5 = "Friends"), ~ valid_numbers(.x) < 5)[[1]]$prop * 100,
     `% low: Things you have` =
      summary_proportions_multiple(pri_test_a, list(lifesat6 = "Things you have"), ~ valid_numbers(.x) < 5)[[1]]$prop * 100,
     `% low: Health` =
      summary_proportions_multiple(pri_test_a, list(lifesat7 = "Health"), ~ valid_numbers(.x) < 5)[[1]]$prop * 100,
     `% low: Appearance` =
      summary_proportions_multiple(pri_test_a, list(lifesat8 = "Appearance"), ~ valid_numbers(.x) < 5)[[1]]$prop * 100,
     `% low: Future` =
      summary_proportions_multiple(pri_test_a, list(lifesat9 = "Future"), ~ valid_numbers(.x) < 5)[[1]]$prop * 100,
     `% low: School` =
      summary_proportions_multiple(pri_test_a, list(lifesat10 = "School"), ~ valid_numbers(.x) < 5)[[1]]$prop * 100,
     `% low: Time use` =
      summary_proportions_multiple(pri_test_a, list(lifesat11 = "Time use"), ~ valid_numbers(.x) < 5)[[1]]$prop * 100,
      `% reporting low mood` =
      ...
      share_elevated(pri_test_a, mme_cat)$prop[2] * 100,
      `% scoring elevated-emotional` =
      share_elevated(pri_test_a, mme_cat)$prop[1] * 100,
    `% scoring as expected-behavioural` =
      share_elevated(pri_test_a, mmb_cat)$prop[2] * 100,
    `% scoring elevated-behavioural` =
      share_elevated(pri_test_a, mmb_cat)$prop[1] * 100,
...
test_that("Excels return values matching report - secondary", {
  ...
  expected <- tibble::tibble(
    ...
    `% low: Overall` =
      summary_proportions_multiple(sec_test_a, list(lifesat1 = "Overall"), ~ valid_numbers(.x) < 5)[[1]]$prop * 100,
    `% low: Family` =
      summary_proportions_multiple(sec_test_a, list(lifesat2 = "Family"), ~ valid_numbers(.x) < 5)[[1]]$prop * 100,
    `% low: Home` =
      summary_proportions_multiple(sec_test_a, list(lifesat3 = "Home"), ~ valid_numbers(.x) < 5)[[1]]$prop * 100,
    `% low: Choice` =
      summary_proportions_multiple(sec_test_a, list(lifesat4 = "Choice"), ~ valid_numbers(.x) < 5)[[1]]$prop * 100,
    `% low: Friends` =
      summary_proportions_multiple(sec_test_a, list(lifesat5 = "Friends"), ~ valid_numbers(.x) < 5)[[1]]$prop * 100,
    `% low: Things you have` =
      summary_proportions_multiple(sec_test_a, list(lifesat6 = "Things you have"), ~ valid_numbers(.x) < 5)[[1]]$prop * 100,
    `% low: Health` =
      summary_proportions_multiple(sec_test_a, list(lifesat7 = "Health"), ~ valid_numbers(.x) < 5)[[1]]$prop * 100,
    `% low: Appearance` =
      summary_proportions_multiple(sec_test_a, list(lifesat8 = "Appearance"), ~ valid_numbers(.x) < 5)[[1]]$prop * 100,
    `% low: Future` =
      summary_proportions_multiple(sec_test_a, list(lifesat9 = "Future"), ~ valid_numbers(.x) < 5)[[1]]$prop * 100,
    `% low: School` =
      summary_proportions_multiple(sec_test_a, list(lifesat10 = "School"), ~ valid_numbers(.x) < 5)[[1]]$prop * 100,
    `% low: Time use` =
      summary_proportions_multiple(sec_test_a, list(lifesat11 = "Time use"), ~ valid_numbers(.x) < 5)[[1]]$prop * 100,
    `% reporting low mood` =
